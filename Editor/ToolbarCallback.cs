...
namespace com.flexford.packages.toolbar
{
	internal static class ToolbarCallback
	{
		...
		static PropertyInfo m_windowBackend = m_guiViewType.GetProperty("windowBackend", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
		static PropertyInfo m_viewVisualTree = m_iWindowBackendType.GetProperty("visualTree", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
#else
			...
			if (m_currentToolbar == null)
			{
				...
				if (m_currentToolbar != null)
				{
					...
					void RegisterCallback(string root, Action cb)
					{
						...
						var parent = new VisualElement() { style =
						{
							flexGrow = 1, 
